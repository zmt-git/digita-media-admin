<template>
  <li
    class="detail-media box-shadow radius-10"
  ...
  >
    <template v-if="!isAddCard">
      ...
      <div class="detail-media-src">
        <el-tag v-if="state && tag" class="state-btn" effect="dark" :type="btnType" size="mini">{{
          btnName
        }}</el-tag>
        <el-image
          fit="contain"
...
export default {
  ...
  computed: {
    img() {
      return !this.isAddCard && this.info.address
        ? this.info.address
        : require('../assets/common/empty.png')
    }
  },
